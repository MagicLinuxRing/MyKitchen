...
#error RSProgressHUD is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

#import "RSProgressHUD.h"
#import <QuartzCore/QuartzCore.h>

NSString * const RSProgressHUDDidReceiveTouchEventNotification = @"RSProgressHUDDidReceiveTouchEventNotification";
NSString * const RSProgressHUDWillDisappearNotification = @"RSProgressHUDWillDisappearNotification";
NSString * const RSProgressHUDDidDisappearNotification = @"RSProgressHUDDidDisappearNotification";

NSString * const RSProgressHUDStatusUserInfoKey = @"RSProgressHUDStatusUserInfoKey";

CGFloat RSProgressHUDRingRadius = 14;
CGFloat RSProgressHUDRingThickness = 6;

@interface RSProgressHUD ()

@property (nonatomic, readwrite) RSProgressHUDMaskType maskType;
...
- (void)showProgress:(float)progress
            ...
            maskType:(RSProgressHUDMaskType)hudMaskType;
...
+ (RSProgressHUD*)sharedView {
    static dispatch_once_t once;
    static RSProgressHUD *sharedView;
...
+ (void)show {
    [[self sharedView] showProgress:-1 status:nil maskType:RSProgressHUDMaskTypeNone];
}

+ (void)showWithStatus:(NSString *)status {
    [[self sharedView] showProgress:-1 status:status maskType:RSProgressHUDMaskTypeNone];
}
...
+ (void)showProgress:(CGFloat)progress {
    [[self sharedView] showProgress:progress status:nil maskType:RSProgressHUDMaskTypeNone];
}

+ (void)showProgress:(CGFloat)progress status:(NSString *)status {
    [[self sharedView] showProgress:progress status:status maskType:RSProgressHUDMaskTypeNone];
}
...
+ (void)showImage:(UIImage *)image status:(NSString *)string {
    NSTimeInterval displayInterval = [[RSProgressHUD sharedView] displayDurationForString:string];
...
- (void)drawRect:(CGRect)rect {
    ...
    switch (self.maskType) {
        ...
        case RSProgressHUDMaskTypeBlack: {
            [[UIColor colorWithWhite:0 alpha:0.5] set];
            CGContextFillRect(context, self.bounds);
            break;
        }
            
        case RSProgressHUDMaskTypeGradient: {
...
- (void)positionHUD:(NSNotification*)notification {
    ...
    if(notification) {
        RSProgressHUD *__weak weakSelf=self;
...
- (void)overlayViewDidReceiveTouchEvent {
    [[NSNotificationCenter defaultCenter] postNotificationName:RSProgressHUDDidReceiveTouchEventNotification object:nil];
}

#pragma mark - Master show/dismiss methods

- (void)showProgress:(float)progress status:(NSString*)string maskType:(RSProgressHUDMaskType)hudMaskType {
    ...
    if(self.maskType != RSProgressHUDMaskTypeNone) {
...
- (void)dismiss {
    NSDictionary *notificationUserInfo = (self.stringLabel.text ? @{RSProgressHUDStatusUserInfoKey : self.stringLabel.text} : nil);
    [[NSNotificationCenter defaultCenter] postNotificationName:RSProgressHUDWillDisappearNotification
                                                        object:nil
    ...
    [UIView animateWithDuration:0.15
                     ...
                     completion:^(BOOL finished){
                         if(weakSelf.alpha == 0) {
                             ...
                             [[NSNotificationCenter defaultCenter] postNotificationName:RSProgressHUDDidDisappearNotification
                                                                                 object:nil
                                                                               userInfo:notificationUserInfo];
...
- (CAShapeLayer *)ringLayer {
    if(!_ringLayer) {
        ...
        _ringLayer = [self createRingLayerWithCenter:center radius:RSProgressHUDRingRadius lineWidth:RSProgressHUDRingThickness color:[UIColor whiteColor]];
...
- (CAShapeLayer *)backgroundRingLayer {
    if(!_backgroundRingLayer) {
        CGPoint center = CGPointMake(CGRectGetWidth(hudView.frame)/2, CGRectGetHeight(hudView.frame)/2);
        _backgroundRingLayer = [self createRingLayerWithCenter:center radius:RSProgressHUDRingRadius lineWidth:RSProgressHUDRingThickness color:[UIColor darkGrayColor]];
...
- (NSTimeInterval)displayDurationForString:(NSString*)string {
    return MIN(1.15, (float)string.length*0.06 + 0.3);
}
...
- (UIImage *)hudSuccessImage {
    ...
    return [UIImage imageNamed:@"RSProgressHUD.bundle/success.png"];
}
...
- (UIImage *)hudErrorImage {
    ...
    return [UIImage imageNamed:@"RSProgressHUD.bundle/error.png"];
}
